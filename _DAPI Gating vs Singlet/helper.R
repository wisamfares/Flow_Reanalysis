...
# Load single ligand experiment data
loadData = function(data_dir){
  ...
  all_data <- csv_files %>%
    ...
    # DAPI Specific for comparisons, can be removed later
    mutate(Gate = if_else(str_detect(Sample_Name, "DAPI"), "DAPI", "Singlet")) %>%
    ...
    select("Sample_Name", "Cell_Line", "Dox", "Ligand", "Ligand_conc", "Replicate", "Gate", contains("FSC-A"),
    ...
    ## Filter outliers 
    group_by(Cell_Line, Ligand, Ligand_conc, Gate) %>%
...
loadTwoLigandData = function(data_dir){
  ...
  all_data <- csv_files %>%
    ...
    # Extract Cell Name
    separate(Sample_Name, into = c("Cell_Line", "TGFB1", "GDF11", "Replicate"), sep = "_", fill = "right", remove = FALSE) %>%
    ...
    # (2) Filter to only include columns of interest, B1.A and YG1.A for 5 laser, HA and pSMAD for 3 laser
    select("Sample_Name", "Cell_Line", "TGFB1", "GDF11", "Replicate", contains("FSC-A"),
           starts_with(old_HA_colname), 
           starts_with(old_pSMAD2_colname),
           starts_with(old_SMAD2_colname),
           starts_with(old_DAPI_colname)) %>%
