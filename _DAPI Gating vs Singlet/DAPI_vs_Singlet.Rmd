...
DAPI_res = run_bootstrap_analysis_pipeline(DAPI_sub, 
                                 pSMAD2_perc_cutoff = 0,
                                 HA_perc_cutoff = 0.95,
                                 n_bins = 10,
                                 n_boot = 1000)

Sing_res = run_bootstrap_analysis_pipeline(Sing_sub,
                                           pSMAD2_perc_cutoff = 0,
                                 HA_perc_cutoff = 0.95,
                                 n_bins = 10,
                                 n_boot = 1000)
...
p = pheatmap(mat,
         ...
         clustering_method = "ward.D2",
         cluster_cols = FALSE,
         main = "-Cyto Binned Analysis",
         show_rownames = FALSE,
         show_colnames = FALSE
         )

saveFigure = function(p, fileName, width = 3, height = 3){
  ggsave(fileName, plot = p, width = width, height = height)
}

saveFigure(p, "DAPI_heatmap.svg")
```
...
```{r, semi-finalized-fig-3H, fig.width = 2, fig.height = 3}
## SEMI-FINALIZED FIGURE 3H
DAPI_minmax = DAPI_binned_boot %>%
  filter(Cell_Line == "R3",
         Ligand != "GDF11",
         Ligand_conc != 10,
...
p = ggplot(DAPI_minmax, aes(x = log10(Ligand_conc), y = range, shape=Experiment))+
  geom_line()+
  geom_point(size=2)+
  ...
  ylab("pSMAD2/SMAD2 Range")+
  xlab("log[TGFB1]")

saveFigure = function(p, fileName, width = 2, height = 2.75){
  ggsave(fileName, plot = p, width = width, height = height)
}

saveFigure(p, "Figure_3H_norm_pSMAD2_Traces.svg")
```
