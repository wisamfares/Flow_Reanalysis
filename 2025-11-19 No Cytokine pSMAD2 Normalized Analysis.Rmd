---
title: "No Cytokine - Norm pSMAD2 Analysis"
author: "Wisam"
date: "2025-11-20"
...
loadData = function(data_dir){
  ...
  all_data <- csv_files %>%
    ...
    # Clean up data frame
    relocate(Sample_Name, Cell_Line, Ligand) %>%
    # (2) Filter to only include columns of interest, B1.A and YG1.A for 5 laser, HA and pSMAD for 3 laser
    select("Sample_Name", "Cell_Line", "Dox", "Ligand", "Replicate", contains("FSC-A"),
...
bootstrapBinnedData = function(binned_data, n_boot){
  ...
  filter(!is.na(HA_bin)) %>%
  group_by(Cell_Line, Dox, Ligand, Replicate, HA_bin) %>%
...
dotPlotBinData = function(binned_data, summ_stat = "mean"){
  ...
  group_by(Cell_Line, Dox, Replicate, Ligand, HA_bin) %>%
...
```{r}
data_dir = "No Cytokine Samples/2024-09-04/"
...
run_bootstrap_analysis_pipeline = function(data_dir, 
  ...
  # (2) Filter Data based on pSMAD2 and HA threshold of Luc - No Cytokine 
    ...
    # (3) Bin Data
    binned_data = filtered_data %>%
      group_by(Ligand, Experiment, Sample_Name) %>%
    ...
    # (4) Bootstrap Binned Data
    binned_boot = binned_data %>%
      group_by(Experiment, Sample_Name) %>%
...
binned_boot = binned_boot %>%
  group_by(Dox, Experiment, Sample_Name) %>%
  arrange(HA_bin) %>%
